...
function Sleep() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:1000/api/user/userinfo')
        ...
        >
          ...
          >
            ...
            >
              ...
              <br />
              {data === null || calculateSleepGoal() === 'Specify a valid age. ' ? (
                ''
              ) : (
                <>{`Recommended Sleep Goal: ${calculateSleepGoal()}  hours`}</>
              )}
            </p>
            <Container
              style={{
            ...
            >
              <Card style={{ width: '50%', height: '100%' }}>
                <Card.Body>
                  ...
                  <Card.Text className='text-muted'>
                    Calculate how many hours of sleep you get every day, what time you should be sleeping to get the
                    best sleep, and much, much more.
                  </Card.Text>
                  <form>
              ...
              {startSleepTime && endSleepTime && (
                <Card style={{ width: '50%', height: '100%', marginTop: '5vh' }}>
                  <Card.Body>
                    <Card.Title>Your Results</Card.Title>
                    <Card.Text>Hours slept: {getSleepTime()}</Card.Text>
                    <Card.Text>
                      Recommended Wake up Times:{' '}
                      {getRecommendedHoursOfSleep().map((e) => (
                        <p>{militaryToStandard(e)}</p>
                      ))}
                    </Card.Text>
                    <Card.Title>Explanation (Sleep Architecture)</Card.Title>
                    <Card.Text>
                      <p>
                        The recommendations above are based on sleep cycles. Although sleep is often considered one
                        event, sleep is actually made up of sleep cycles, which are in turn made up of sleep stages.
                      ...
                      <p>
                        Sleep cycles can be improved by improving the environment in which you sleep, having a
                        consistent sleep schedule,
                        {data.personal.age >= 21 ? ' reducing alcohol consumption before bed, ' : ''} and touching grass
